function renderArticleCard(data) {
    let article_card = `<div class="deck_article-card-container">
...
function renderGridDeckCards(container, decks, mode, clearContainer = true) {
    const ctr = jQuery(container);
    if (clearContainer === true) {
        ctr.empty();
    }
    for (const deck of decks) {
        const elem = jQuery('<div class="deck_article-card-container">' +
            '<a>' +
        ...
        elem.find('.deck_article-card').attr('title', deck.deck_name).attr('data-src', 'https://images.ygoprodeck.com/images/cards_cropped/' + coverCard + '.jpg');
        elem.find('.deck_article-card-title').text(deck.deck_name);
        if (mode === 'tournament' || isTournament) {
            elem.find('.deck-cost-wrapper').remove();
            if (deck.tournamentName) {
                elem.find('.deck-type-badge').text(deck.tournamentName);
                elem.find('.deck_article-card-excerpt').remove();
                elem.find('.deck_article-card-stats').addClass('d-block').html('<span class="mr-1"></span><span></span>');
                elem.find('.deck_article-card-stats').children().first().html('<i class="fa-solid fa-trophy" aria-hidden="true"></i> ').appendText(deck.tournamentPlacement + ' (' + (deck.tournamentPlayerCountIsApproximate ? '~' : '') + deck.tournamentPlayerCount + ' players)');
                elem.find('.deck_article-card-stats').children().eq(1).html(' <i class="fa-regular fa-calendar" aria-hidden="true"></i> ').appendText(deck.submit_date);
                if (deck.tournamentPlayerName) {
                    elem.find('.deck_article-card-stats').append(' piloted by <span class="ml-1"><i class="fa-solid fa-user" aria-hidden="true"></i> ' + deck.tournamentPlayerName + '</span>');
                }
            } else {
                elem.find('.deck-type-badge').remove();
                elem.find('.deck_article-card-excerpt').text(deck.deck_excerpt);
                elem.find('.deck_article-card-stats').remove();
            }
        } else {
            elem.find('.deck-type-badge').text(deck.format);
            elem.find('.deck-cost-wrapper').children().first().appendText(deck.deck_price ? ' $' + deck.deck_price : ' -');
            elem.find('.deck-cost-wrapper').children().eq(1).html('<img src="https://images.ygoprodeck.com/images/cards/icons/master_duel/super_rare.png" class="md-craft-icons" title="Master Duel Super Rare Craft Cost"> ').appendText(deck.super || '-').append(' <img src="https://images.ygoprodeck.com/images/cards/icons/master_duel/ultra_rare.png" class="md-craft-icons" title="Master Duel Ultra Rare Craft Cost"> ').appendText(deck.ultra || '-');
            elem.find('.deck_article-card-excerpt').text(deck.deck_excerpt);
            elem.find('.deck_article-avatar').html('<img src="' + (deck.img_url || '') + '" title="' + deck.username + ' avatar" loading="lazy">');
            elem.find('.deck_article-card-stats').children().first().appendText(deck.username + ' - ' + deck.submit_date);
            elem.find('.deck_article-views-comments').children().first().appendText(deck.deck_views.toLocaleString());
            elem.find('.deck_article-views-comments').children().eq(1).appendText(deck.comments);
        }
    }
    ctr.find('.lazy').Lazy({
        defaultImage: "https://images.ygoprodeck.com/images/assets/CardBack.jpg",
...
function getPieChart(sel) {
    ...
    if (!elm) {
        console.warn('Piechart with selector \'' + sel + '\' not found.');
        return null;
    }
...
function renderPieSlice(sel, label, img, value, onClick) {
    const pie = getPieChart(sel);
    if (!pie) return;
    const slice = jQuery('<span class="piechart-slice"></span>').prop('title', label + ' (' + value + ')').css('background-image', img);
    if (value < pie.piechartTotal) {
        ...
        slice.css('clip-path', 'polygon(' + clipPath.join(',') + ')').css('background-size', backgroundSize * 100 + '% ' + backgroundSize * 100 + '%').css('background-position-x', 'calc(' + leftBackgroundImageEdge + ' * var(--piechart-size))').css('background-position-y', 'calc(' + topBackgroundImageEdge + ' * var(--piechart-size))');
    } else {
...
function renderLongHex(item) {
    ...
    const formatDate = new Date(item.date_created);
    const cubeDate = formatDate.toLocaleDateString('en-US', {month: 'short', day: 'numeric', year: 'numeric'});
    const hex = `<div class="hex-content p-2">
